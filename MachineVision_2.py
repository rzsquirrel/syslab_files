...
WIDTH = 400 #PLUG IN FROM FILE
HEIGHT = 400 #PLUG IN FROM FILE
...
def main():
    file1 = open('triangle_circle.ppm', 'r')    #filename here
    stng = file1.readline()
    for i in range(2):
        stng += file1.readline()
    print(stng)
    ...
    #voteImage = findLinePoints(sharpEdges,GxMatrix,GyMatrix)
...
def findCircleCenter(image,GxMatrix,GyMatrix):
    ...
    for pos in range(len(image)):
        r = pos//WIDTH
        c = pos%WIDTH
        if image[pos] == 0 and GxMatrix[pos] !=0: 
            slope = -GyMatrix[pos]/GxMatrix[pos]
            b = r-c*slope
            voteMatrix = castVotes(slope,b,voteMatrix)
    
    maxCell = max(voteMatrix)
    maxIndex = voteMatrix.index(maxCell)
    
    for i in range(len(voteMatrix)):
        if voteMatrix[i]<0.9*maxCell:
            voteMatrix[i] = 0
    
    R = 1
    
    for r in range(WIDTH-maxIndex%WIDTH):
        print(image[r+maxIndex])
        if image[r+maxIndex] < 255:
            R = r
            break
    
    drawCircle(maxIndex%WIDTH,maxIndex//WIDTH,R,voteMatrix,maxCell)
...
def castVotes2(m,b,voteMatrix,image):
    if abs(m) < 1:
        for x in range(WIDTH):
            index = x+WIDTH*int(m*x+b)
            if 0 <= index < len(image) and image[index]!=255:
                voteMatrix[index] += 1
    else:
        for y in range(HEIGHT):
            index = int((y-b)/m) + WIDTH*y
...
def drawCircle(Cx,Cy,r,image,value):
    print(r)
    for s in range(360):
        t = s*3.14159/180
        x = int(Cx+r*cos(t))
...
def findLinePoints(image,GxMatrix,GyMatrix):
    voteMatrix = [0] * HEIGHT * WIDTH
    for pos in range(len(image)):
        r = pos//WIDTH
        c = pos%WIDTH
        if image[pos] == 0 and GyMatrix[pos] != 0:
            for i in range(3):
                slope = GxMatrix[pos]/GyMatrix[pos] + (i-1)*0.02
                b = r-c*slope
                #castVotes(slope,b,voteMatrix)
                castVotes2(slope,b,voteMatrix,image)
        '''
        elif GyMatrix[pos] == 0:
            for y in range(HEIGHT):
                image[r+y*WIDTH] += 1
        '''
    #maybe threshold  at 0.5 * darkest point?
    maxCell = max(voteMatrix)
    for i in range(len(voteMatrix)):
        if voteMatrix[i]<0.3*maxCell:
            voteMatrix[i]=0
    return voteMatrix
