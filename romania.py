...
nodes=open('romEdges.txt').read().split() #0-1 adj, 2-3 adj, etc.
latlong=open('romNodes.txt').read().split() #(id, lat, long,id2,lat2,long2,...)
nodeToCity=open('romFullNames.txt').read().split()
...
def storeGraphUnw():
   ...
   fout=open('romGraphUnweighted.pkl','wb')
...
def storeGraphW():
   ...
   fout=open('romGraphWeighted.pkl','wb')
   dump(edgeListW,fout)
   fout.close()
...
def search(start,end):
   ...
   while q:
      #print(q)
      popped=getMin(q)
      #print(popped)    #popped is a tuple, (length so far, [(d1,node1),(d2,node2),...])
      pops+=1
      last=popped[1][-1][1] #last is just the id string
      #print(last)
      if last==end:
      ...
      for nbr in edgeListW[last]: #nbr is (distance,id)
         inUsed=False
         newPath=copy.deepcopy(popped) #newPath is a tuple (len so far, [(d1,node1),(d2,node2),...])
         ...
         while i<len(used) and inUsed==False:
            #print(nbr[1])
            if used[i][1][-1][1]==nbr[1]:
               inUsed=True
               #print('repeat')
               if used[i][0]>newPath[0]:
                  used.pop(i)
                  q.append(newPath)
            i+=1
         if inUsed==False:
...
def main():
   mapToName={}
   for word in nodeToCity:
      mapToName[word[0]]=word
   #storeGraphUnw()
   #storeGraphW()
   #print the first 20
   """
   ...
   while 1==1:
      ...
      if endWord=='x':
         exit()
      path=search(startWord[0],endWord[0])
      for i in range(len(path[0][1])):
         print(1+i,'.',mapToName[path[0][1][i][1]])
