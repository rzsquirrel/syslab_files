...
def modifyColors(ant, matrix):
    cellColor = int(matrix[ant[2]][ant[1]])
    #should work for any n colors
    if HEADING_RULE[cellColor] == 0:
        ant[0]=(ant[0]+1)%4    #left
    elif HEADING_RULE[cellColor] == 1:
        ant[0]=(ant[0]-1)%4    #right
    #change color, should work for any n colors
    newColorNum = (cellColor+1)%NUMBER_OF_COLORS
    matrix[ant[2]][ant[1]] = newColorNum
    
    canvas.create_rectangle(ant[1],ant[2],ant[1]+5,ant[2]+5,fill=COLORS[newColorNum])

def makeTheAntsJourney(ant, matrix):
    message = "PROGRAM CURRENTLY RUNNING " + str(STEPS) + " and moves in " + str(SPEED_INC) + " increments."
    print(message)
    root.title(message)
    startTime = clock()
...
#HEADING_RULE = (0,1) #two colors, (part 1)
HEADING_RULE = (0,1,1,0) #four colors, should look symmetric (part 2)
#HEADING_RULE = (0,0,1,1,0,0) #six colors
