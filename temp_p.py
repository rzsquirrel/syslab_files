...
class TheFrame(tk.Frame):
    def __init__(self, root):
        ...
        for element in elArray:
            if element[0] >= 0 and element[1] >= 0 and element[2] >= 0 and element[3] >= 0:
                self.canvas.create_polygon(xyArray[element[0]][1], xyArray[element[0]][2], xyArray[element[1]][1], xyArray[element[1]][2], xyArray[element[2]][1], xyArray[element[2]][2], xyArray[element[3]][1], xyArray[element[3]][2], outline = "black", fill = "red", activefill = "pink")
            else:
                self.canvas.create_polygon(600, 600)
        print(self.canvas.create_polygon(10,10,10,20,20,10,20,20,outline="black",fill="red"))
    ...
    def motion(self,event):
        ...
        #shadeNeighbors()
        theItem = self.canvas.find_closest(self.canvas.canvasx(event.x), self.canvas.canvasy(event.y))[0]
        tag = self.canvas.gettags(theItem)[0]
        print(tag)
