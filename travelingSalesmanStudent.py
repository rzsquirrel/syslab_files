...
def setUpCanvas(root): # These are the REQUIRED magic lines to enter graphics mode.
    root.title("THE TRAVELING SALESMAN PROBLEM by Reed Zhang.")
...
#---3. Sort on y-coordinate and connect sequentially by y.
    #city = sorted(city,key=lambda tup: tup[2])
#---4. Sort on x-coordinate and connect sequentially by x.
    #city = sorted(city,key=lambda tup: tup[1])
#---5. Your algorithm(s). Can you do better than the sorting algorithms above?
    '''
    temp = sorted(city,key=lambda tup: tup[1])
    city = []
    assert(len(city)==0 and len(temp)==n)
    city.append(temp[0])
    temp.remove(temp[0])
    assert(len(city)==1 and len(temp)==n-1)
    while temp:
        choices = []
        for i in range(min(3,len(temp))):
            choices.append(temp[i])
        choices = sorted(choices,key=lambda tup: tup[2])
        city.append(choices[0])
        temp.remove(choices[0])
    assert(len(city)==n)
    '''
    city = sorted(city,key=lambda tup: tup[2])
    #take points left of the line and put them at the end sorted by reverse tup[2]
    top,bot = city[0],city[-1]
    slope = (top[2]-bot[2])/(top[1]-bot[1])
    intercept = top[2]-slope*top[1]
    leftPoints = []
    for point in city:
        if point[1]<(point[2]-intercept)/slope:
            leftPoints.append(deepcopy(point))
            city.remove(point)
    leftPoints = sorted(leftPoints,key = lambda tup:tup[2])
    leftPoints.reverse()
    for element in leftPoints:
        city.append(element)
    assert(len(city)==n)
#---6. Display results.
